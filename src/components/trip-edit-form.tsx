...
export function TripEditForm({ data }: { data: TripEditData }) {
  ...
  return <form className="editor-layout" onSubmit={submit}><aside className="editor-steps"><Link href={`/viaggi/${data.id}`}><ChevronLeft size={15} /> Torna al viaggio</Link><p className="nav-label">Modifica viaggio</p></aside><div className="editor-main"><div className="editor-heading"><div><p className="eyebrow">Configurazione</p><h1>Modifica viaggio</h1><p>Aggiorna informazioni, date, capienza e quote.</p></div></div>{error ? <div className="form-error form-error-block">{error}</div> : null}<input type="hidden" name="tripId" value={data.id} />
    <section className="form-card"><div className="form-card-title"><span>01</span><div><h2>Informazioni</h2></div></div><div className="form-grid"><label className="form-span-2"><span>Titolo *</span><input name="title" defaultValue={data.title} required /></label><label><span>Codice *</span><input name="code" defaultValue={data.code} pattern="[A-Za-z0-9-]{3,20}" required /></label><label><span>Destinazione *</span><input name="destination" defaultValue={data.destination} required /></label><label className="form-span-2"><span>Descrizione</span><textarea name="description" defaultValue={data.description} rows={4} /></label></div></section>
