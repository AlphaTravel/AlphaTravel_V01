...
function RoomsBoard({ trip, data, canManage }: { trip: Trip; data: TripOperationsData; canManage: boolean }) {
  ...
  return <div className="workspace-grid room-workspace"><section className="panel unassigned-panel"><div className="panel-header"><div><p className="eyebrow">{unassigned.length} persone</p><h2>Da assegnare</h2></div><Users size={19} /></div>{unassigned.length ? unassigned.map((participant) => <div className="draggable-person" key={participant.registrationId}><span>{participant.initials}</span><p><strong>{participant.name}</strong><small>{participant.roomPreference}</small></p></div>) : <div className="empty-inline">Tutte le persone sono assegnate.</div>}</section><section className="panel panel-span-2"><div className="panel-header"><div><p className="eyebrow">{data.accommodations.length} strutture</p><h2>Camere del viaggio</h2></div>{canManage ? <Link className="button button-secondary" href={`/viaggi/${trip.id}/logistica?section=rooms`}><Settings2 size={15} /> Gestisci camere</Link> : null}</div><div className="room-grid">{data.rooms.length ? data.rooms.map((room) => <div className={cn("room-card", room.guests.length >= room.capacity && "room-full")} key={room.id}><div><span><BedDouble size={16} /> {room.accommodationName} · {room.number}</span><StatusBadge label={room.isAccessible ? "Accessibile" : room.type} /></div><p>{room.guests.map((guest) => <strong key={guest.registrationId}>{guest.name}</strong>)}</p><small>{room.guests.length}/{room.capacity} posti · piano {room.floor}</small></div>) : <div className="empty-inline">Nessuna camera configurata.</div>}</div></section></div>;
}
