...
export type LogisticsSection = "participants" | "rooms" | "transport" | "schedule";

export function TripLogisticsManager({ trip, data, initialSection = "participants" }: { trip: Trip; data: TripOperationsData; initialSection?: LogisticsSection }) {
  const router = useRouter();
  const [activeSection, setActiveSection] = useState<LogisticsSection>(initialSection);
  ...
  const sections: Array<{ key: LogisticsSection; label: string; description: string; count: string; icon: typeof Users }> = [
    { key: "participants", label: "Partecipanti", description: "Gruppi e iscrizioni", count: `${data.participants.length} iscritti`, icon: Users },
    { key: "rooms", label: "Camere", description: "Strutture e assegnazioni", count: `${data.rooms.length} camere`, icon: BedDouble },
    { key: "transport", label: "Trasporti", description: "Mezzi e posti", count: `${data.vehicles.length} mezzi`, icon: BusFront },
    { key: "schedule", label: "Programma", description: "Attività e camminate", count: `${data.itinerary.length} attività`, icon: CalendarDays },
  ];

  function selectSection(section: LogisticsSection) {
    setActiveSection(section);
    setMessage(null);
    router.replace(`/viaggi/${trip.id}/logistica?section=${section}`, { scroll: false });
  }

  return (
    <div className="admin-sections logistics-manager">
      <div className="detail-nav"><Link href={`/viaggi/${trip.id}`}>← Torna alla panoramica</Link></div>
      <header className="editor-heading"><div><p className="eyebrow">Organizzazione del viaggio</p><h1>{trip.title}</h1><p>Scegli una sezione e modifica soltanto le informazioni necessarie.</p></div></header>
      {message ? <div className={message.ok ? "success-banner" : "form-error form-error-block"}>{message.text}</div> : null}

      <nav className="logistics-tabs" aria-label="Sezioni organizzazione viaggio">
        {sections.map(({ key, label, description, count, icon: Icon }) => (
          <button className={activeSection === key ? "logistics-tab active" : "logistics-tab"} type="button" aria-current={activeSection === key ? "page" : undefined} onClick={() => selectSection(key)} key={key}>
            <span><Icon size={19} /></span><strong>{label}</strong><small>{description}</small><b>{count}</b>
          </button>
        ))}
      </nav>

      {activeSection === "participants" ? <section className="logistics-section" aria-labelledby="logistics-participants-title">
      <div className="logistics-section-heading"><div><p className="eyebrow">01 · Partecipanti</p><h2 id="logistics-participants-title">Gruppi e iscrizioni</h2></div><p>Crea prima gli eventuali gruppi, poi iscrivi i pellegrini al viaggio.</p></div>
      <div className="logistics-section-grid">
      <form className="form-card" method="post" onSubmit={(event) => submit(event, "group", addTripGroupAction)}>
      ...
      <form className="form-card" method="post" onSubmit={(event) => submit(event, "registration", registerPilgrimAction)}>
      ...
      {activeSection === "rooms" ? <section className="logistics-section" aria-labelledby="logistics-rooms-title">
      <div className="logistics-section-heading"><div><p className="eyebrow">02 · Camere</p><h2 id="logistics-rooms-title">Strutture, camere e assegnazioni</h2></div><p>Configura nell’ordine la struttura, le camere disponibili e infine gli ospiti.</p></div>
      <div className="logistics-section-grid">
      <form className="form-card" method="post" onSubmit={(event) => submit(event, "accommodation", addAccommodationAction)}>
      ...
      <form className="form-card" method="post" onSubmit={(event) => submit(event, "room", addRoomAction)}>
      ...
      <form className="form-card logistics-form-wide" method="post" onSubmit={(event) => submit(event, "room-assignment", assignRoomAction)}>
      ...
      {activeSection === "transport" ? <section className="logistics-section" aria-labelledby="logistics-transport-title">
      <div className="logistics-section-heading"><div><p className="eyebrow">03 · Trasporti</p><h2 id="logistics-transport-title">Mezzi e posti</h2></div><p>Aggiungi un mezzo; i posti vengono creati automaticamente e sono subito assegnabili.</p></div>
      <div className="logistics-section-grid">
      <form className="form-card" method="post" onSubmit={(event) => submit(event, "vehicle", addVehicleAction)}>
      ...
      <form className="form-card" method="post" onSubmit={(event) => submit(event, "seat-assignment", assignSeatAction)}>
      ...
      {activeSection === "schedule" ? <section className="logistics-section" aria-labelledby="logistics-schedule-title">
      <div className="logistics-section-heading"><div><p className="eyebrow">04 · Programma</p><h2 id="logistics-schedule-title">Attività e camminate</h2></div><p>Inserisci una voce alla volta con orari, luogo e alternative accessibili.</p></div>
      <div className="logistics-section-grid">
      <form className="form-card logistics-form-wide" method="post" onSubmit={(event) => submit(event, "itinerary", addItineraryItemAction)}>
